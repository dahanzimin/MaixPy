    ...
    else
    {
        ...
        { // Read OV sensor ID.
            ...
            case OV9650_ID:
                ...
                break;
            case OV2640_ID:
                mp_printf(&mp_plat_print, "Find ov2640\n");
                init_ret = ov2640_init(sensor);
                break;
            case OV5640_ID:
                mp_printf(&mp_plat_print, ":Find ov5640\n");
            ...
            //     break;
            case OV5642_ID:
                mp_printf(&mp_plat_print, "Find ov5642\n");
                init_ret = ov5642_init(sensor);
                break;
            case OV7740_ID:
                mp_printf(&mp_plat_print, "Find ov7740\n");
                init_ret = ov7740_init(sensor);
                break;
            case OV3660_ID:
                mp_printf(&mp_plat_print, "Find ov3660\n");
...
{
    uint16_t id = 0;
    // mp_printf(&mp_plat_print, "Find gc sensor\n");
    if (pwnd)
    ...
    {
        mp_printf(&mp_plat_print, "Sensor id = %x\n", id);
        sensor->snapshot = sensor_snapshot;
        sensor->flush = sensor_flush;
        sensor->chip_id = id;
        switch (sensor->chip_id)
        ...
        case GC0328_ID:
            ...
            mp_printf(&mp_plat_print, "Find gc0328\n");
            gc0328_init(sensor);
            break;
        case GC2145_ID:
            mp_printf(&mp_plat_print, "Find gc2145\n");
...
int sensro_mt_detect(sensor_t *sensor, bool pwnd)
{
    ...
    {
        // mp_printf(&mp_plat_print, "Find mt9d111\n");
...
int sensor_init_dvp(mp_int_t freq, bool default_freq)
{
    ...
    {
        // find ov sensor
        mp_printf(&mp_plat_print, "Find ov sensor\n");
        pwdn_lock = 1;
    }
    ...
    {
        //find mt sensor
        mp_printf(&mp_plat_print, "Find mt sensor\n");
        cambus_set_writeb_delay(2);
    }
    else
    {
        mp_printf(&mp_plat_print, "No sensor\n");
        init_ret = -1;
    }
    // mp_printf(&mp_plat_print, "Limit = %x sensor.choice_dev = %x \n", limit, sensor.choice_dev);
...
int sensor_reset(mp_int_t freq, bool default_freq, bool set_regs, bool double_buff, uint8_t choice_dev)
{
    ...
    {
        // mp_printf(&mp_plat_print, "Sensor reset function is null\n");
        return -1;
    }
    ...
    // mp_printf(&mp_plat_print, "Exit sensor_reset\n");
    return 0;
}
...
int binocular_sensor_scan()
{
    ...
    if (sensor.slv_addr == 0)
    {
        mp_printf(&mp_plat_print, "Can not find sensor first\n");
        /* Sensor has been held in reset,
           so the reset line is active low */
        sensor.reset_pol = ACTIVE_LOW;
        /* Pull the sensor out of the reset state,systick_sleep() */
    ...
    {
        mp_printf(&mp_plat_print, "Sensors don't match\n");
        return -2;
    }
    // Clear sensor chip ID.
    sensor.chip_id = 0;
    ...
    else
    {
        ...
        else
        {
            ...
            case OV7725_ID:
                /*ov7725_init*/
            default:
                mp_printf(&mp_plat_print, "Sensor is not supported.");
                return -3;
            }
        }
    }
...
int binocular_sensor_reset(mp_int_t freq)
{
    ...
    else
    {
        ...
        {
            //find gc0328 sensor
            mp_printf(&mp_plat_print, "Sensor1 find gc0328\n");
            cambus_set_writeb_delay(2);
        }
        else
        {
            mp_printf(&mp_plat_print, "Scan sensor1 error\n");
            return -1;
        }
        DCMI_PWDN_LOW();
        if (0 == sensro_gc_detect(&sensor, false))
        {
            //find gc0328 sensor
            mp_printf(&mp_plat_print, "Sensor2 find gc0328\n");
            cambus_set_writeb_delay(2);
        }
        else
        {
            mp_printf(&mp_plat_print, "Scan sensor2 error\n");
            return -1;
        }
    }
    ...
    if (sensor.reset(&sensor) != 0)
    { //rst reg, set default cfg.
        mp_printf(&mp_plat_print, "First sensor reset failed\n");
        return -1;
    }
    ...
    if (sensor.reset(&sensor) != 0)
    { //rst reg, set default cfg.
        mp_printf(&mp_plat_print, "Second sensor reset failed\n");
        return -1;
    }
...
int sensor_sleep(int enable)
{
    if (sensor.sleep == NULL || sensor.sleep(&sensor, enable) != 0)
    {
        ...
        mp_printf(&mp_plat_print, "Operation not supported.");
        return -1;
    }
    return 0;
}
...
int sensor_snapshot(sensor_t *sensor, image_t *image, streaming_cb_t streaming_cb, bool update_jb)
{
    ...
    do
    {
        ...
        if (MAIN_FB()->bpp > 3)
        {
            mp_printf(&mp_plat_print, "%s | bpp error\n", __func__);
            return -1;
        }
