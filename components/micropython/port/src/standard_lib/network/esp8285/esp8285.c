...
bool get_host_byname(esp8285_obj* nic, const char* host,uint32_t len,char* out_ip, uint32_t timeout_ms)
{
	...
	{
		mp_printf(&mp_plat_print, "%s | get_host_byname failed\n",__func__);
		return false;
	}
	char IP_buf[16]={0};
...
bool get_ipconfig(esp8285_obj* nic, ipconfig_obj* ipconfig)
{
	...
	if(cur == NULL)
	{
		mp_printf(&mp_plat_print, "%s | esp8285_ipconfig could'n get ip\n",__func__);
		return false;
	}
	char ip_buf[16] = {0};
	...
	if(cur == NULL)
	{
		mp_printf(&mp_plat_print, "%s | esp8285_ipconfig could'n get gateway\n",__func__);
		return false;
	}
	...
	if(cur == NULL)
	{
		mp_printf(&mp_plat_print, "%s | esp8285_ipconfig could'n get netmask\n",__func__);
		return false;
	}
