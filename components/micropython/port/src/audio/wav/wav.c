...
mp_obj_t wav_play_process(audio_t* audio,uint32_t file_size)
{
	...
	{
		mp_printf(&mp_plat_print, "Can not create decode object\n");
	...
	if(err_code != 0)
	{
		mp_printf(&mp_plat_print, "Read head error close file\n");
		m_del(mp_obj_list_t,ret_list,1);
		m_del(wav_decode_t,audio->play_obj,1);
		vfs_internal_close(audio->fp,&close_code);
	...
	if(status != OK)
	{
		mp_printf(&mp_plat_print, "WAV error code : %d\n",status);
		m_del(mp_obj_list_t,ret_list,1);
		m_del(wav_decode_t,audio->play_obj,1);
		vfs_internal_close(audio->fp,&close_code);
	...
	mp_printf(&mp_plat_print, "Numchannels = %d\n", wav_fmt->numchannels);
	mp_printf(&mp_plat_print, "Samplerate = %d\n", wav_fmt->samplerate);
	mp_printf(&mp_plat_print, "Byterate = %d\n", wav_fmt->byterate);
	mp_printf(&mp_plat_print, "Blockalign = %d\n", wav_fmt->blockalign);
	mp_printf(&mp_plat_print, "Bitspersample = %d\n", wav_fmt->bitspersample);
	mp_printf(&mp_plat_print, "Datasize = %d\n", wav_fmt->datasize);
	// mp_printf(&mp_plat_print, "Head_len = %d\n", head_len);
	...
	if(err_code != 0)
	{
		mp_printf(&mp_plat_print, "Seek error  close file\n");
		m_del(mp_obj_list_t,ret_list,1);
		m_del(wav_decode_t,audio->play_obj,1);
		vfs_internal_close(audio->fp,&close_code);
		mp_raise_OSError(err_code);
...
mp_obj_t wav_play(audio_t* audio)
{
	...
	if(play_obj->audio_buf[play_obj->read_order].empty)//empty ,altread to read
	{
		...
		// mp_printf(&mp_plat_print, "Read_order = %d\n",play_obj->read_order);
	...
	{
		// mp_printf(&mp_plat_print, "Play_order = %d\n",wav_play_obj->play_order);
...
mp_obj_t wav_record_process(audio_t* audio,uint32_t channels)//channels = Number of channels
{
	...
	{
		mp_printf(&mp_plat_print, "Can not create encode object\n");
...
void wav_finish(audio_t* audio)
{
	...
	if(audio->record_obj != NULL)
	{
		...
		if(err_code != 0)
		{
			mp_printf(&mp_plat_print, "Write file chunk error  close file\n");
		...
		if(err_code != 0)
		{
			mp_printf(&mp_plat_print, "Write formate chunk error close file\n");
			wav_record_buf_free(wav_encode);
			m_del(wav_encode_t,audio->record_obj,1);
			vfs_internal_close(audio->fp,&close_code);
			mp_raise_OSError(err_code);
		...
		if(err_code != 0)
		{
			mp_printf(&mp_plat_print, "Write data chunk error  close file\n");
			wav_record_buf_free(wav_encode);
			m_del(wav_encode_t,audio->record_obj,1);
			vfs_internal_close(audio->fp,&close_code);
			mp_raise_OSError(err_code);
