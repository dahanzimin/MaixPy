...
static uint8_t sd_get_cardinfo(SD_CardInfo *cardinfo)
{
    ...
    {
        debug_print("%s | sd_get_csdregister failed\r\n",__func__);
        return 0xFF;
    }
    if (sd_get_cidregister(&(cardinfo->SD_cid)))
    {
        debug_print("%s | sd_get_cidregister failed\r\n",__func__);
        return 0xFF;
    }
    if (2 == sd_version)
    {
...
uint8_t sd_init(void)
{
    ...
    while (index--)
    {
        ...
        sd_read_data(frame, 4);
        sd_end_cmd();
        debug_print("%s |  frame[0] = %x \r\n", __func__, frame[0]);
        debug_print("%s |  frame[1] = %x \r\n", __func__, frame[1]);
        debug_print("%s |  frame[2] = %x \r\n", __func__, frame[2]);
        debug_print("%s |  frame[3] = %x \r\n", __func__, frame[3]);
        debug_print("%s |  result = %d \r\n", __func__, result);
        debug_print("%s |  index = %d \r\n", __func__, index);
        if (result == 0)
        {
            break;
        }
