    ...
    while 1:
        ...
        if key.value() == 0:
            ...
            if key.value() == 0 and (last_btn_status == 1) and (time.ticks_ms() - last_cap_time > 500):
                ...
                img.draw_string(0,0,"Please B-key release",(255,0,0),2,mono_space=0)
        else:
            time.sleep_ms(30)
            if key.value() == 1 and (last_btn_status == 0):
                last_btn_status = 1
            if cap_num < class_num:
                img.draw_string(0,0, "Press B-key: "+class_names[cap_num],(255,0,0),2,mono_space=0)
            elif cap_num < class_num + sample_num:
                img.draw_string(0,0,  "Press B-key: NO.{}".format(cap_num-class_num),(255,0,0),2,mono_space=0)
                       
        if cap_num >= class_num + sample_num:
            print("start train")
            classifier.train()
            img.draw_string(12,88,"Training...",(255,0,0),4,mono_space=0)
            lcd.display(img)   
            time.sleep_ms(1000)   
            print("train end") 
            img = image.Image()
            img.draw_string(25,88,"Train OK",(255,0,0),4,mono_space=0)  
            lcd.display(img) 
