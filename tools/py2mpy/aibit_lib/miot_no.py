...
def do_connect(account, password):
	try:
		...
		import network_no
		network_no.do_connect(account, password)

def ntp(url='http://time1909.beijing-time.org/time.asp'):
	data = request.get(url,headers={'User-Agent': 'Mozilla/5.0'}).text.split("\r\n")
	return (int(data[1][6: -1]),int(data[2][7: -1]),int(data[3][5: -1]),int(data[4][6: -1]),int(data[5][5: -1]),int(data[6][5: -1]),int(data[7][5: -1]))
