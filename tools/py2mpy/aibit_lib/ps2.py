...
class PS2Controller:
	def __init__(self,clk_pin,do_pin,di_pin,cs_pin,mode=1,timeout=1000):  #mode: 0 red; 1 green
		self.di =board.pin(di_pin, board.GPIO.IN)
		self.do =board.pin(do_pin, board.GPIO.OUT)
		self.cs =board.pin(cs_pin, board.GPIO.OUT)
		self.clk=board.pin(clk_pin, board.GPIO.OUT)
