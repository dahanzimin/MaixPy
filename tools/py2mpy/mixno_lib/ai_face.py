...
def init(FD=None, LD=None, FE=None):
	global task_fd
	global task_ld
	global task_fe	
	task_fd = kpu.load(0x500000) if FD is None else kpu.load(FD)
	task_ld = kpu.load(0x600000) if LD is None else kpu.load(LD)
	task_fe = kpu.load(0x700000) if FE is None else kpu.load(FE)
	
	gc.collect()	
	key_gpio = board.pin(24,board.GPIO.IN,board.GPIO.PULL_UP)
	...
	if code:
		for i in code:
			...
			if max_score > threshold:
				...
				if bb==1:
					print("[MixNo]: Please press A-key to enter the face") 
					bb=0
